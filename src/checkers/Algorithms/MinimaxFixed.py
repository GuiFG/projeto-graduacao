...
class MinimaxFixed():
    ...
    def max_value(self, state, depth):
        ...
        for action in state.actions(self.player):
            new_state = copy.deepcopy(state)
            state_result = new_state.result(action, self.player)
            
            value, move = self.min_value(state_result, depth + 1)
    ...
    def min_value(self, state, depth):
        ...
        for action in state.actions(op_player):
            new_state = copy.deepcopy(state)
            state_result = new_state.result(action, op_player)
